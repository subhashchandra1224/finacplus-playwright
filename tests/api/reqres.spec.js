 ...
 * via GET using the exact same ID.
 *
 ...
 *
 * API workflow:
 *   POST   /api/collections/{slug}/records        → 201, record created, dynamic id returned
 *   GET    /api/collections/{slug}/records/{id}   → 200, same record retrieved and validated
 *   PUT    /api/collections/{slug}/records/{id}   → 200, record updated, validated
 *   DELETE /api/collections/{slug}/records/{id}   → cleanup after tests
 *
 * Response envelope:
...
test.describe('ReqRes API — Create → Get Created User → Update', () => {
  ...
  test.beforeAll(() => {
    if (!config.reqres.projectKey) {
      throw new Error(
        ...
        '  5. Set: REQRES_COLLECTION=products'
      );
    }
  });
  ...
  // Validate 201, response contains name/job, store dynamic id
  // -------------------------------------------------------------------------
  ...
  // Validates the persisted record matches exactly what was created.
  // -------------------------------------------------------------------------
  test('GET created user — validate 200 and persisted data matches POST payload', async ({ request }) => {
    ...
    expect(record.id, 'GET id must match POST id').toBe(createdRecordId);
